...
class TestCreateModel:
    ...
    def test_set_member_called_four_times(self, mock_create_grillage, mock_lpm, bridge):
        self._setup_bridge(bridge, edge_dist=1.1)
        mock_model = mock_create_grillage.return_value
        bridge.create_model()
        # _assign_girder_members uses the mocked model so 0 per-girder calls;
        # explicit assignments: edge_beam + transverse_slab + start_edge + end_edge = 4
        assert mock_model.set_member.call_count == 4, f"set_member should be called 4 times, got {mock_model.set_member.call_count}"
    ...
    def test_edge_beam_not_assigned_when_no_overhang(self, mock_create_grillage, mock_lpm, bridge):
        """When edge_dist=0 the code skips the edge_beam set_member call entirely."""
        self._setup_bridge(bridge, edge_dist=0)
        mock_model = mock_create_grillage.return_value
        bridge.create_model()
        # No call should use member="edge_beam" when edge_dist=0
        edge_beam_calls = [c for c in mock_model.set_member.call_args_list if c.kwargs.get("member") == "edge_beam"]
        assert len(edge_beam_calls) == 0, (
            f"set_member should NOT be called with member='edge_beam' when edge_dist=0, but got {len(edge_beam_calls)} call(s)"
        )
...
class TestDeadLoadCombination:
    ...
    def test_dl_combination_load_count_matches_sub_cases(self, mock_lc, bridge):
        ...
        load_groups entries across all non-None sub-cases in _DL_ATTRS.
        The source iterates: self_weight_load_case, deck_load_case, footpath_load_case,
        sidl_combination — if create_dead_load_combination() skips or double-counts
        any of these, this count diverges."""
        bridge.model = MagicMock()
        ...
        bridge.sidl_combination = _make_lc(3)  # sidl_combination IS in _DL_ATTRS
        bridge.footpath_load_case = None

        expected_total = sum(
            len(lc.load_groups)
            for lc in [bridge.self_weight_load_case, bridge.deck_load_case, bridge.sidl_combination]
            if lc is not None
        )
...
@patch("osdagbridge.core.bridge_types.plate_girder.analyser.og.plot_force", create=True)
@patch("osdagbridge.core.bridge_types.plate_girder.analyser.og.plot_defo", create=True)
@patch("osdagbridge.core.bridge_types.plate_girder.analyser.og.plt", create=True)
class TestPlot:
...
class TestCreateGoverningLLLoadCase:
    ...
    @patch("osdagbridge.core.bridge_types.plate_girder.analyser.og.create_envelope")
    def test_governing_ll_load_case_creation_flow(self, mock_create_envelope, mock_lc):
        bridge = BridgeGrillageModel()
        bridge.model = MagicMock()

        # Setup dataset with two static load cases
        dataset = MagicMock()
        dataset.coords = {"Loadcase": MagicMock(values=["case 1", "case 2"])}

        # Patch og.create_envelope to return "case 1" as the governing case
        # create_envelope is called twice: once for value_mode, once for query_mode
        env_val_chain = MagicMock()
        env_lc_chain = MagicMock()

        # value-mode chain: .get().sel(Component="Mz_i") -> xarray-like with idxmax / sel
        val_da = MagicMock()
        val_da.idxmax.return_value.item.return_value = 101   # governing element tag
        val_da.sel.return_value.values = 50000.0
        env_val_chain.get.return_value.sel.return_value = val_da

        # query-mode chain: .get().sel(Component="Mz_i") -> da where .sel(Element=101).values = "case 1"
        lc_da = MagicMock()
        lc_da.sel.return_value.values = "case 1"
        env_lc_chain.get.return_value.sel.return_value = lc_da

        mock_create_envelope.side_effect = [env_val_chain, env_lc_chain]
