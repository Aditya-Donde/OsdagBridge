    ...
    if log_entries:
        for entry in log_entries:
            for raw_line in entry.split('\n'):
                ...
                upper = line.upper()
                if 'WARNING' in upper:
                    lines_tex.append(rf'\textcolor{{blue}}{{{escaped}}}\\')
                elif 'ERROR' in upper:
                    lines_tex.append(rf'\textcolor{{red}}{{{escaped}}}\\')
                elif 'INFO' in upper:
                    lines_tex.append(rf'\textcolor{{osdagGreen}}{{{escaped}}}\\')
                else:
                    continue  # skip lines without a known level
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if os.path.exists(osdag_logo_src):
                osdag_dest = os.path.join(assets_dir, 'Osdag Logo.png')
                shutil.copy2(osdag_logo_src, osdag_dest)
                osdag_logo_latex = 'assets/Osdag Logo.png'
