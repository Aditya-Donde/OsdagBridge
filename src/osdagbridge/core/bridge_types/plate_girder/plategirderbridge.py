...
def resolve_cb_value(source: dict, base_key: str, default=None, pair: str | None = None):
    """
    Resolve a cross-bracing property from an input dict, tolerating both the
    per-member dynamic key scheme and the legacy flat key.

    Per-member values are stored under ``<base_key>.G{i}G{i+1}.B{i}M{k}``
    (see ``defaults.extend_cb_dynamic_keys``); the No.-of-Cross-Bracings count
    is stored at pair level (``<base_key>.G{i}G{i+1}``). Resolution order:

      1. ``<base_key>.{pair}.B{i}M1`` — the requested pair (only when ``pair``
         is given, e.g. ``"G1G2"``),
      2. the first per-member/per-pair key found (keys sorted, so G1G2.B1M1 is
         the representative fallback),
      3. the legacy flat ``<base_key>``,
      4. ``default``.

    ``None`` and empty strings count as missing; ``False`` is a real value.
    """
    def _present(v) -> bool:
        return v is not None and v != ""

    if pair:
        m = re.match(r"G(\d+)G\d+", pair)
        if m:
            val = source.get(f"{base_key}.{pair}.B{m.group(1)}M1")
            if _present(val):
                return val
            val = source.get(f"{base_key}.{pair}")
            if _present(val):
                return val

    prefix = base_key + "."
    for key in sorted(k for k in source if k.startswith(prefix)):
        if _present(source[key]):
            return source[key]

    val = source.get(base_key)
    return val if _present(val) else default
...
class PlateGirderBridge:
    ...
    def add_wind_loads(self) -> None:
        ...
        # Values pasted to output_dict. Vz / Pz already arrive from the
        # Additional Inputs compute step, so they are not recomputed here.
        self.output_dict[KEY_WL_BASIC_WIND_SPEED] = f"{basic_wind_speed:.2f}"
    ...
    def add_temperature_load(self) -> None:
        ...
        self.output_dict[KEY_TL_HIGHEST_MAX_TEMP]    = max_temp
        self.output_dict[KEY_TL_LOWEST_MIN_TEMP]     = min_temp
        self.output_dict[KEY_TL_THERMAL_COEFF_STEEL] = inp.get(KEY_TL_THERMAL_COEFF_STEEL)
        self.output_dict[KEY_TL_THERMAL_COEFF_RCC]   = inp.get(KEY_TL_THERMAL_COEFF_RCC)
    ...
    def add_seismic_loads(self) -> None:
        ...
        self.output_dict[KEY_SL_SEISMIC_ZONE]     = zone_name
    ...
    def _store_load_combinations_for_widget(self) -> None:
        ...
        the default list. Must run before create_uls/sls_combinations(), which
        name their load cases from it (see _lc_name_by_key). The expansion lives alongside the load-combination
        widget that renders it (single source of truth) — imported lazily so this
        module keeps importing when the desktop UI is unavailable.
        """
        from osdagbridge.desktop.ui.dialogs.additional_input.ui_builder._load_combination_widget import (
            LoadCombinationWidget,
        )

        self.output_dict[KEY_ALL_LOAD_COMBINATIONS] = LoadCombinationWidget.build_load_combinations_widget(
            self.input_dict.get(self._LC_SELECTION_KEY),
            self.input_dict.get(KEY_LC_COMBINATIONS),
        )

    def _lc_name_by_key(self) -> dict:
        """Widget display name per combination key. The analyser names its
        load cases with these, so cache keys, force-table columns and the
        load-combinations table all carry the identical string."""
        return {c["key"]: c["name"] for c in self.output_dict.get(KEY_ALL_LOAD_COMBINATIONS) or [] if c.get("key")}
    ...
    def create_uls_combinations(self) -> list:
        ...
        return self.grillage_model.create_uls_combinations(included_keys=included, name_by_key=self._lc_name_by_key())
    ...
    def create_sls_combinations(self) -> list:
        ...
        return self.grillage_model.create_sls_combinations(included_keys=included, name_by_key=self._lc_name_by_key())
    ...
    def _design_end_diaphragm_members(self) -> dict:
        ...
        dict — nested by pair → member → force_type → Osdag result.
        """
        from osdagbridge.core.bridge_types.plate_girder.crossbracingforces import EndDiaphragmForces
    ...
    def get_3d_cad_parameters(self) -> BridgeParametersDTO:
        ...
        _cb_type_raw = str(resolve_cb_value(inp, KEY_MP_CB_TYPE, "") or "").strip().upper()
        cb_bracing_type = "K" if _cb_type_raw.startswith("K") else "X"
        ...
        try:
            ...
            for table in ("EqualAngle", "UnequalAngle"):
                ...
                if not row:
                    ...
                    row = cur.fetchone()
                if not row and len(nums) >= 3:
                    # Fallback to LIKE with numbers — only for angle-shaped
                    # designations (3 numbers); a 1-number channel like "JC 100"
                    # must not LIKE-match an angle and shadow the Channels table.
                    cur.execute(
