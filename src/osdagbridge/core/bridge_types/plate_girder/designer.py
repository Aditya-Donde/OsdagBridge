...
class BridgeConfig:
    ...
    def from_plate_girder_bridge(cls, bridge: Any, girder_index: int | None = None) -> "BridgeConfig":
        ...
        # ones: the Girder Details dialog writes them into the input dict when the
        # IS section is picked (AdditionalInputs._on_is_section_changed), the same
        # way the welded dimensions arrive, so analysis, design and CAD all size
        # the same section.
        designation = ""
        if fabrication == "rolled":
            is_section = str(_gv(inp, KEY_MP_GIRDER_IS_SECTION, girder_index)).strip()
