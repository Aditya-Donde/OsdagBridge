...
def get_flat_location_options(db: Database, include_placeholder: bool = True) -> List[str]:
    options: List[str] = []
    for state in db.get_all_states() or []:
...
def get_default_location() -> Dict[str, str]:
    """Pick a state/station that has windspeed data so lookup shows values by default."""
    db = Database(DB_PATH)
    db.connect()
    try:
        row = db.cursor.execute(
            "SELECT state, station FROM windspeed_data ORDER BY state, station LIMIT 1"
        ).fetchone()
        if row:
            return {"state": row[0], "station": row[1]}
    finally:
        db.close()

    # Fallback: first state + first station
    states = get_state_list(include_placeholder=False)
    if not states:
        return {"state": "", "station": ""}
    first_state = states[0]
    stations = get_station_list(first_state, include_placeholder=False)
    first_station = stations[0] if stations else ""
    return {"state": first_state, "station": first_station}
