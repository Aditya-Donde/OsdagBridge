...
class BridgeInputValidator:
    ...
    def validate_basic_inputs(self, key: str, inputs: dict) -> dict:
		...
		# ----------------------------------
            ...
            median_included = (
                median is True
                or str(median).strip().lower() in {"yes", "true", "1"}
            )
            
            if carriageway_width is None:
                min_w = CARRIAGEWAY_WIDTH_MIN_WITH_MEDIAN if median_included else CARRIAGEWAY_WIDTH_MIN
                return min_w, "Carriageway width must be specified."
            
            assumed_lanes = 2 if median_included else 1
