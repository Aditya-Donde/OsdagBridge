...
def _add_coordinate_triad(ax, nodes, scale=0.25):
    ...
    # Compute box_aspect to know screen-space size per axis
    try:
    ...
    # Target: arrows occupy 'scale' fraction of the smaller of bx, bz screen dims
    target_screen = min(bx, bz) * scale

    # Convert target screen fraction → data units for each axis independently
    # screen_units_per_data_unit = box_dim / data_range
    # data_units_for_target = target_screen / (box_dim / data_range)
    Lx = target_screen * xr / bx
    Lz = target_screen * zr / bz

    # Arrowhead: same screen-space proportion for both
    HL_x = Lx * 0.30
    HL_z = Lz * 0.30
    HW_x = Lx * 0.15
    HW_z = Lz * 0.15
