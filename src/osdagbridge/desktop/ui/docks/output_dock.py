...
from osdagbridge.core.utils.common import (
    ...
    KEY_STEELDESIGN_LOAD_COMBINATION, KEY_STEELDESIGN_MEMBER_ID,
...
class OutputDock(QWidget):
    ...
    def refresh_loadcase_dropdowns(self):
        ...
        for key in (KEY_ANALYSIS_LOAD_COMBINATION, KEY_STEELDESIGN_LOAD_COMBINATION):
            combo = self._w(key)
    ...
    def refresh_member_dropdown(self):
        ...
        if not self.backend or not hasattr(self.backend, "input_dict"):
            return
        combo = self._w(KEY_STEELDESIGN_MEMBER_ID)
        if combo is None:
            return
    ...
    def connect_design_dropdowns(self):
        ...
        for key in (KEY_STEELDESIGN_MEMBER_ID, KEY_STEELDESIGN_LOAD_COMBINATION):
            combo = self._w(key)
            if combo is not None:
    ...
    def _on_design_selection_changed(self):
        ...
        if not self.backend or not hasattr(self.backend, "get_dcr_for_selection"):
            return
        member_combo = self._w(KEY_STEELDESIGN_MEMBER_ID)
        lc_combo     = self._w(KEY_STEELDESIGN_LOAD_COMBINATION)
