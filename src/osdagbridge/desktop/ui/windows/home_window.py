"""
Home window for OsdagBridge GUI.
...
from osdagbridge.desktop.ui.components.svg_card import SvgCardContainer
from osdagbridge.desktop.ui.components.home.navbar import VerticalMenuBar
from osdagbridge.desktop.ui.utils.custom_buttons import MenuButton
from osdagbridge.desktop.ui.components.home.top_right_buttons import TopButton, DropDownButton
from osdagbridge.desktop.ui.components.home.home_widget import HomeWidget
from osdagbridge.desktop.ui.utils.custom_cursors import pointing_hand_cursor


# --- Theme Toggle Button ---
class ThemeToggleButton(QPushButton):
    """Placeholder button - dark mode is still under development."""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedSize(50, 50)
        self.setObjectName("themeToggle")
        self.setCursor(pointing_hand_cursor())
        self.setIcon(QIcon(":/vectors/day_button.svg"))
...
class BackgroundSvgWidget(QWidget):
    def __init__(self, svg_light, parent=None):
        super().__init__(parent)
        # Ensures automatic deletion when closed
        self.setAttribute(Qt.WA_DeleteOnClose, True)
        self.renderer = QSvgRenderer(svg_light)
        self.pixmap = None
    ...
    def updatePixmap(self):
        ...
        # Draw SVG background
        self.renderer.render(painter, QRectF(0, 0, size.width(), size.height()))
        # Draw left border
        pen = painter.pen()
...
class HomeWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.content = BackgroundSvgWidget(":/vectors/background_light.svg", parent=self)
        ...
        self._load_branding_svgs()
        self.show_home()

    def _load_branding_svgs(self):
        """Load the fixed branding artwork once.

        These used to be re-loaded from the resource system on every paintEvent,
        which reran on every hover and resize even though the artwork never
        changes.
        """
        self.top_svg_widget_1.load(":/vectors/OsdagBridge_label_light.svg")
        self.middle_top_svg_widget.load(":/vectors/OsdagBridge_tagline_light.svg")
