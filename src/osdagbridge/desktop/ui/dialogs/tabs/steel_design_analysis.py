...
#  Design tokens ΓÇö copied verbatim from output_dock.apply_field_style()
...
# Interactive field ΓÇö e.g. the x-position field (white, editable)
_FIELD_STYLE_WHITE = """
    QLineEdit {
        padding: 1px 7px;
        border: 1px solid black;
...
# Combo box ΓÇö exact match for output_dock.apply_field_style(QComboBox)
_COMBO_STYLE = """
    QComboBox {
        padding: 1px 7px;
        border: 1px solid black;
...
# Outer panel/card borders ΓÇö same grey as input/output docks
_CARD_STYLE = (
    "QFrame#girderCard { "
    "background-color: white; "
...
# Inner QGroupBox (Support Reactions / Maximum Values) ΓÇö matches _analysis_group_style
_GROUP_STYLE = """
    QGroupBox {
        font-weight: bold;
        font-size: 11px;
...
class SteelDesignAnalysisTab(QWidget):
    ...
    QScrollArea
    ΓööΓöÇ container (QVBoxLayout)
       ΓööΓöÇ main_row (QHBoxLayout)
          Γö£ΓöÇ left_panel  (fixed 330px)
          Γöé   Γö£ΓöÇ Selection card  [Member | Load Combo]
          Γöé   ΓööΓöÇ Results card    [Reactions | Maxima groups]
          ΓööΓöÇ diagram_card  (expanding)
              Γö£ΓöÇ diagram_top_bar  [ΓöÇΓöÇcenterΓöÇΓöÇ comp_combo  spacing  mode_group ΓöÇΓöÇcenterΓöÇΓöÇ]
              Γö£ΓöÇ separator
              ΓööΓöÇ inner_row
                 Γö£ΓöÇ diagram_placeholder  (FigureCanvas injected at runtime)
                 ΓööΓöÇ right_col  [x_input | M_z | V_y | D_y]
    """
    ...
    # ΓöÇΓöÇ WIDGET FACTORY HELPERS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    def _card_frame(self):
        """Outer panel card ΓÇö consistent grey rounded border."""
        frame = QFrame()
    ...
    def _section_title(self, text):
        """Bold section header ΓÇö matches label style in other docks."""
        lbl = QLabel(text)
        lbl.setStyleSheet(
    ...
    def _build_left_panel(self):
        ...
        # for 130(label)+8(spacing)+160(field)=298px ΓÇö 4px breathing room each side
        ...
        # ΓöÇΓöÇ Selection card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Results card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        for key, label in [
            ...
            self._form_row(label, field, reac_inner)
    ...
    # ΓöÇΓöÇ DIAGRAM SECTION ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    def _build_diagram_section(self):
        """
        Right-hand panel containing:
          1. top_bar  ΓÇö Component combo + Display Location group, both centered
          2. separator
          3. inner_row ΓÇö FigureCanvas placeholder + RHS value column
        """
        ...
        # ΓöÇΓöÇ Component titled card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        comp_card = QFrame()
        ...
        # ΓöÇΓöÇ Display Location titled card placeholder ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Inner row: plots + RHS value column ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        inner_row = QHBoxLayout()
        inner_row.setSpacing(10)
        inner_row.setContentsMargins(0, 0, 0, 0)

        # Diagram placeholder ΓÇö replaced at runtime with FigureCanvas
        ...
        self.x_input.setPlaceholderText("click plotΓÇª")
        ...
        # subplots_adjust: top=0.94, bottom=0.12  ΓåÆ usable height Γëê 475 px (at 580 px canvas)
        #
        # Subplot vertical centres from canvas top (px, ~580 px canvas):
        #   ax_scheme centre Γëê  50 px   (x_input top-pad = 35 gets mid at ~49 px)
        #   ax_bmd    centre Γëê 154 px   ΓåÆ gap after x_input row Γëê 74 px
        #   ax_sfd    centre Γëê 303 px   ΓåÆ gap after M_z row     Γëê 119 px
        #   ax_defl   centre Γëê 451 px   ΓåÆ gap after V_y row     Γëê 119 px
        #   bottom pad                  Γëê 114 px
    ...
    def set_girder_count(self, count):
        """Repopulate member_combo with 'Girder 1' ΓÇª 'Girder N' entries."""
