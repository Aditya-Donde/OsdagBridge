...
class EndDiaphragmDetailsTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.cross_connection_type_combo = None
    ...
    def bind_additional_input_instance(self, additional_input_instance) -> None:
        """Bind the AdditionalInputs owner for Typical Section synchronization."""
        self._additional_input_instance = additional_input_instance
    ...
    def _girder_pairs(self) -> list[str]:
        girders = []
        ai = getattr(self, "_additional_input_instance", None)
        if ai is not None and hasattr(ai, "typical_section_tab"):
            try:
                count_text = str(getattr(ai.typical_section_tab, "no_of_girders", None).text() or "").strip()
                count = int(float(count_text)) if count_text else None
            except Exception:
                count = None
            if count is not None and count >= 2:
                girders = [f"G{i}" for i in range(1, count + 1)]
        if not girders and self._girder_details_tab is not None and hasattr(self._girder_details_tab, "available_girders"):
            try:
    ...
    def _end_schema_overview_fields(self, view_key: str) -> list[dict]:
        """Return normalized `overview` fields for one view."""
        views = END_DIAPHRAGM_DETAILS_SCHEMA.get("views", {})
        view_schema = views.get(str(view_key or ""), {}) if isinstance(views, dict) else {}
        fields = view_schema.get("overview", [])
        return [dict(field) for field in fields if isinstance(field, dict)]
    ...
    def _open_welded_thickness_values_dialog(self, field_key: str) -> None:
        ...
        dialog = _ThicknessSelectionDialog(titles.get(field_key, "Select Values"), selected, SAIL_APPROVED_THICKNESS_VALUES, self)
        if dialog.exec() != QDialog.Accepted:
            return
    ...
    def _build_cross_bracing_view(self):
        ...
        for field_def in self._end_schema_section_inputs(VIEW_CROSS_BRACING):
            field_id = str(field_def.get("id") or "").strip()
            if field_id == "design":
                continue
            widget = self._schema_builder.create_widget(field_def)
            ...
            if field_id == "bracing_type":
                ...
                self.cross_connection_type_combo = widget
            elif field_id == "bracing_section_type":
                self.cross_bracing_section_type_combo = widget
            elif field_id == "bracing_section":
                self.cross_bracing_section_combo = widget
            elif field_id == "top_chord_enabled":
                self.cross_top_chord_checkbox = widget
            elif field_id == "top_chord_type":
                self.cross_top_chord_type_combo = widget
            elif field_id == "top_chord_size":
                self.cross_top_chord_size_combo = widget
            elif field_id == "bottom_chord_enabled":
                self.cross_bottom_chord_checkbox = widget
            elif field_id == "bottom_chord_type":
                self.cross_bottom_chord_type_combo = widget
            elif field_id == "bottom_chord_size":
                self.cross_bottom_chord_size_combo = widget
        ...
        # Wire up dynamic designations + previews (same behavior as before).
        if isinstance(design_combo, QComboBox):
            design_combo.currentTextChanged.connect(self._on_cross_design_changed)

        if isinstance(self.cross_bracing_type_combo, QComboBox):
            self.cross_bracing_type_combo.currentTextChanged.connect(self._on_cross_bracing_layout_changed)
        if isinstance(self.cross_top_chord_checkbox, QCheckBox):
            self.cross_top_chord_checkbox.toggled.connect(self._on_cross_bracing_layout_changed)
        if isinstance(self.cross_bottom_chord_checkbox, QCheckBox):
            self.cross_bottom_chord_checkbox.toggled.connect(self._on_cross_bracing_layout_changed)

        if isinstance(self.cross_bracing_section_type_combo, QComboBox):
            self.cross_bracing_section_type_combo.currentTextChanged.connect(
                lambda label: (
                    self._cross_update_designations_for(self.cross_bracing_section_combo, label),
                    self._update_cross_previews(),
                )
            )
        if isinstance(self.cross_bracing_section_combo, QComboBox):
            self.cross_bracing_section_combo.currentTextChanged.connect(self._update_cross_previews)

        if isinstance(self.cross_top_chord_type_combo, QComboBox):
            self.cross_top_chord_type_combo.currentTextChanged.connect(
                lambda label: (
                    self._cross_update_designations_for(self.cross_top_chord_size_combo, label),
                    self._update_cross_previews(),
                )
            )
        if isinstance(self.cross_top_chord_size_combo, QComboBox):
            self.cross_top_chord_size_combo.currentTextChanged.connect(self._update_cross_previews)

        if isinstance(self.cross_bottom_chord_type_combo, QComboBox):
            self.cross_bottom_chord_type_combo.currentTextChanged.connect(
                lambda label: (
                    self._cross_update_designations_for(self.cross_bottom_chord_size_combo, label),
                    self._update_cross_previews(),
                )
            )
        if isinstance(self.cross_bottom_chord_size_combo, QComboBox):
            self.cross_bottom_chord_size_combo.currentTextChanged.connect(self._update_cross_previews)

        self._cross_populate_designations()
        self._on_cross_design_changed(design_combo.currentText() if isinstance(design_combo, QComboBox) else "")
    ...
    def _build_rolled_view(self):
        ...
        rolled_inputs = []
        for field_def in self._end_schema_section_inputs(VIEW_ROLLED_BEAM):
            field_id = str(field_def.get("id") or "").strip()
            if field_id == "design":
                continue
            widget = self._schema_builder.create_widget(field_def)
            ...
            if field_id == "is_section" and isinstance(field_widget, QComboBox):
                self.rolled_is_section_combo = field_widget
                rolled_inputs.append(field_widget)

        self._rolled_inputs = rolled_inputs
        ...
        if hasattr(self, "rolled_is_section_combo") and self.rolled_is_section_combo:
            self.rolled_is_section_combo.currentTextChanged.connect(self._update_rolled_preview_and_props)
        self._update_rolled_preview_and_props()
    ...
    def _build_welded_view(self):
        ...
        welded_inputs = []
        for field_def in self._end_schema_section_inputs(VIEW_WELDED_BEAM):
            field_id = str(field_def.get("id") or "").strip()
            if field_id == "design":
                continue
            ...
            if field_type == "combo":
                if isinstance(widget, QComboBox):
                    welded_inputs.append(widget)
            elif field_type == "dimension":
                bind = str(field_def.get("bind") or "").strip()
                field_widget = getattr(self, bind, None) if bind else None
                if isinstance(field_widget, QLineEdit):
                    welded_inputs.append(field_widget)
            elif field_type == "mode_value":
                mode_combo = getattr(self, str(field_def.get("bind_mode") or "").strip(), None)
                value_input = getattr(self, str(field_def.get("bind_value") or "").strip(), None)
                if isinstance(mode_combo, QComboBox):
                    welded_inputs.append(mode_combo)
                if isinstance(value_input, QLineEdit):
                    welded_inputs.append(value_input)

        self._welded_inputs = welded_inputs
        ...
        for watcher in (self.welded_total_depth, self.welded_top_width, self.welded_bottom_width):
            watcher.textChanged.connect(self._update_welded_preview_and_props)
        for watcher in (
            self.welded_web_thickness_value,
            self.welded_top_thickness_value,
            self.welded_bottom_thickness_value,
        ):
            watcher.textChanged.connect(self._update_welded_preview_and_props)
        self.welded_web_thickness_combo.currentTextChanged.connect(
            lambda text: self._on_welded_thickness_mode_changed("welded_web_thickness", text)
        )
        self.welded_top_thickness_combo.currentTextChanged.connect(
            lambda text: self._on_welded_thickness_mode_changed("welded_top_thickness", text)
        )
        self.welded_bottom_thickness_combo.currentTextChanged.connect(
