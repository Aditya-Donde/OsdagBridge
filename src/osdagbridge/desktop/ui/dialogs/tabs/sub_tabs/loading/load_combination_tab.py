...
class LoadCombinationTab(QWidget):
    ...
    def _refresh_load_combo_table(self):
        ...
        for idx, combo in enumerate(self.load_combo_items):
            ...
            checkbox.setStyleSheet(
                "QCheckBox { spacing: 0px; background-color: transparent; }"
            )
        ...
        new_height = max(180, min(header_height + (row_count * row_height) + 8, 260))
    ...
    def _open_load_combo_dialog(self, existing=None):
        ...
        custom_items = []
        for item in getattr(self.owner, "custom_load_items", []):
        ...
        for btn in (add_btn, modify_btn, delete_btn):
            ...
            btn.setStyleSheet(
                "QPushButton { background: #ffffff; border: 1px solid #a0a0a0; border-radius: 4px; padding: 6px 12px; "
                "font-size: 11px; font-weight: 500; color: #2a2a2a; }"
        ...
        for btn in (cancel_btn, save_btn):
            ...
            btn.setStyleSheet(
                "QPushButton { background: #c8c8c8; border: 1px solid #a0a0a0; border-radius: 4px; padding: 8px 16px; "
                "font-weight: 600; font-size: 11px; color: #2a2a2a; }"
