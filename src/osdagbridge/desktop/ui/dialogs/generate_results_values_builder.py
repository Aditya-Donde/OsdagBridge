...
def available_load_cases(output_dict: dict) -> list:
    ...
    "1.0 DL", "Basic_1 : 1.35DL + …" — the names the by-load-case force tables
    are built from), then any combination from KEY_ALL_LOAD_COMBINATIONS the
    analysis did not run. Combinations carry the identical name in both places
    (the analyser names them from the widget list), so plain string equality works.
    """
    od = output_dict or {}
    names = list(dict.fromkeys(
        str(lc) for lc_data in (od.get("load_effects_cache") or {}).values() for lc in (lc_data or {})
    ))
    names += [c["name"] for c in (od.get(KEY_ALL_LOAD_COMBINATIONS) or [])
