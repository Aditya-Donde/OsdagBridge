...
class MainWindow(QMainWindow):
    ...
    def _update_sidebar_visibility(self):
        ...
        if not hasattr(self, 'sidebar'):
            return
        if not isinstance(self.main_widget_instance, HomeWindow): # Some CustomWindow Objects (To include Plugin Objects)
            self.sidebar.resize_sidebar(self.width(), self.height())
            self.sidebar_y = self.title_bar.height() + (self.tab_widget.height() - self.sidebar.height()) // 2
            self.sidebar_animation.stop()
            self._sidebar_expanded = False
    ...
    def _check_design_done(self, index) -> bool:
        ...
        if hasattr(module, 'backend'):
            return module.backend.design_completed
        else:
            return False
    ...
    def _close_tab(self, index):
        ...
        if template_instance and hasattr(template_instance, 'cad_widget'):
            try:
                ...
                try:
                    from .OS_safety_protocols import AISContextLock, get_cleanup_coordinator
                    with AISContextLock():
                        coordinator = get_cleanup_coordinator()
                ...
                if result == "Locate Osi":
                    ...
                    if id is not None:
                        try:
                            ...
                            import shutil
                            report_folder = f"osdag_home/data/reports/file_{id}"
                            try:
